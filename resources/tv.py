...
class TvSeasons(TvInfo):   ####Gets number of season for parsing episodes
    def __init__(self, tv_title, tv_year):
        ...
        self.season_query_string = '{base_url}tv/{tv_id}?api_key={api_key}'.format(base_url=self.base_url, tv_id=self.id, api_key=self.api_key)
...
class TvEpisodes(TvSeasons):   ####Will return a dict of all data for all seasons and episodes
    def __init__(self, tv_title, tv_year):
        ...
        for i in range(self.num_of_seasons):
            self.episode_query_string = '{base_url}tv/{tv_id}/season/{season_number}?api_key={api_key}'.format(base_url=self.base_url, tv_id=self.id, season_number=(i+1), api_key=self.api_key)
